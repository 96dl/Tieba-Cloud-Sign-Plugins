...
function callback_install() {
	global $m;
	$m->query("
		CREATE TABLE IF NOT EXISTS `".DB_PREFIX."ver4_zdwk_log` (
...
function callback_init() {
	...
	option::set('ver4_zdwk_day',0);
	cron::set('ver4_zdwk_dopost','plugins/ver4_zdwk/cron/dopost.php',0,0,0);
}
...
function callback_inactive() {
	option::del('ver4_zdwk_pid');
	option::del('ver4_zdwk_day');
	cron::del('ver4_zdwk_dopost');
}
...
function callback_remove() {
	//在这里做点事
	global $m;
	$m->query("DROP TABLE IF EXISTS `".DB_PREFIX."ver4_zdwk_log`");
}
