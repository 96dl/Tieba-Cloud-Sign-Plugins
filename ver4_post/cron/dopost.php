...
function collect($b,$u,$t){
    ...
    if (!empty($randtime)) $nextdo = $now + randNum($t['space']);
    ...
    while ($rc = $m->fetch_array($c)) $content[] = $rc['content']; //循环列出用户回帖内容以供随机筛选
    if (count($content) < 1) {
        $uc = $m->query("SELECT * FROM `" . DB_NAME . "`.`" . DB_PREFIX . "ver4_post_content` WHERE `tid` = 0 AND `uid` = '{$t['uid']}'");  //查询用户设置的回帖内容
        while ($urc = $m->fetch_array($uc)) $ucontent[] = $urc['content']; //循环列出用户回帖内容以供随机筛选
        if (count($ucontent) < 1) {
            $con = getTuLing();
        } else {
    ...
    if (!empty($t['qid'])) $con = substr($con, 0, 60);
