...
if (isset($_GET['newtieba'])) {
	...
	foreach ($rc as $k => $v){
		$v = str_replace("\n",'',$v);
		$v = str_replace("\r",'',$v);
		if (empty($v))unset($rc[$k]);
	}
	$rc = array_values($rc);

	if (empty($pid) || count($rc) < 1 || empty($tieba) || empty($space)) redirect('index.php?plugin=ver4_review&error=' . urlencode('信息不完整，添加失败！'));

	$time = option::get('ver4_review_time');
	if ($space < $time || $space > 86400) redirect('index.php?plugin=ver4_review&error=' . urlencode('间隔设置不正确！'));

	$p = $m->fetch_array($m->query("SELECT * FROM `" . DB_NAME . "`.`" . DB_PREFIX . "baiduid` WHERE `id` = '{$pid}'"));
	if ($p['uid'] != UID) redirect('index.php?plugin=ver4_review&error=' . urlencode('你不能替他人添加需要扫描的贴吧'));
	...
	if ($t['c'] >= $limit) redirect('index.php?plugin=ver4_review&error=' . urlencode('啊哦，您只能添加' . $limit . '个扫描吧哦！'));
...
if (isset($_GET['ctieba'])) {
	...
	if (!is_array($rc) || empty($space)) redirect('index.php?plugin=ver4_review&error=' . urlencode('信息不完整，添加失败！'));
	if ($space < $time || $space > 86400) redirect('index.php?plugin=ver4_review&error=' . urlencode('间隔设置不正确！'));
	$p = $m->fetch_array($m->query("SELECT * FROM `" . DB_NAME . "`.`" . DB_PREFIX . "ver4_review_list` WHERE `id` = '{$id}'"));
	if ($p['uid'] != UID) redirect('index.php?plugin=ver4_review&error=' . urlencode('你不能修改他人的贴吧设置'));
